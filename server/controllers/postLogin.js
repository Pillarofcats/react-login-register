async function postLogin (req, res, dbPool, bcryptjs) {
  ...
  try {
    ...
    if(passMatch) {
      ...
      return res.cookie('user', email, { domain: 'https://react-register-login-production.up.railway.app', maxAge: 300000, secure: true, sameSite:'None', expires: new Date(Date.now() + 8 * 3600000) })
                .status(200)
